...
class TrainArgumentsMixin:
    ...
    # SMILES token-level augmentation settings (for molecular data training)
    smiles_token_mask_enabled: bool = field(
        default=False,
        metadata={
            'help': 'Whether to enable random masking of SMILES tokens during training.'
        }
    )
    smiles_token_mask_ratio: float = field(
        default=0.15,
        metadata={
            'help': 'The ratio of SMILES tokens to mask (0.0-1.0). Default is 0.15.'
        }
    )
    smiles_token_dropout_enabled: bool = field(
        default=False,
        metadata={
            'help': 'Whether to enable random dropout of SMILES token embeddings during training.'
        }
    )
    smiles_token_dropout_ratio: float = field(
        default=0.1,
        metadata={
            'help': 'The ratio of SMILES token embeddings to dropout (0.0-1.0). Default is 0.1.'
        }
    )
    smiles_special_tokens: Optional[List[str]] = field(
        default=None,
        metadata={
            'help': 'List of special tokens to exclude from SMILES augmentation.'
        }
    )
...
class SwiftArgumentsMixin(RLHFArgumentsMixin, TrainArgumentsMixin):
    ...
    Args:
        ...
        problem_type (Optional[str]): Required for classification models (i.e., when task_type is 'seq_cls'). Can be
            'regression', 'single_label_classification', 'multi_label_classification', or 'multitask'. Defaults to
            None, which is resolved to 'regression' if the model is a reward_model or num_labels is 1, and
            'single_label_classification' otherwise. When set to 'multitask', use '--num_cls_labels' and
            '--num_reg_labels' to specify the number of classification and regression labels respectively, and
            '--multitask_loss_weight' to control the weight of regression loss in the combined loss.
        """
...
class VllmArguments:
    ...
    def get_vllm_engine_kwargs(self):
        ...
        if self.task_type and (self.task_type in ('embedding', 'seq_cls') or 'reranker' in self.task_type):
