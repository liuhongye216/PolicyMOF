...
class SwiftMixin:
    ...
    def __init__(self,
        ...
        self._patch_tasks()
        self._init_multitask_loss_weighter()  # Initialize dynamic loss weighting
        self._init_smiles_augmenter()  # Initialize SMILES token augmentation
    ...
    def _init_multitask_loss_weighter(self):
        """Initialize dynamic loss weighting for multitask training."""
        # Check if model is configured for multitask
        ...
        strategy = getattr(model_config, 'multitask_loss_strategy', 'fixed')
        if strategy == 'fixed':
            # Use fixed weight, no need to create weighter
            return
        
        # Import and create the loss weighter
        from .multitask_loss import create_multitask_loss_weighter
        
        dwa_temperature = getattr(model_config, 'dwa_temperature', 2.0)
        device = str(self.args.device) if hasattr(self.args, 'device') else 'cuda'
        
        weighter = create_multitask_loss_weighter(
            strategy=strategy,
            device=device,
            dwa_temperature=dwa_temperature,
        )
        
        # Add weighter parameters to optimizer if using uncertainty weighting
        if strategy == 'uncertainty':
            # The weighter's parameters need to be optimized
            # We add them to the model so they're included in optimizer
            self.model.multitask_loss_weighter = weighter
            logger.info(f'Initialized multitask loss weighter with strategy: {strategy}')
        else:
            logger.info(f'Initialized multitask loss weighter with strategy: {strategy}, temperature: {dwa_temperature}')

    def _init_smiles_augmenter(self):
        """Initialize SMILES token augmentation for molecular data training."""
        self.smiles_augmenter = None
        
        # Check if SMILES augmentation is enabled via args
        mask_enabled = getattr(self.args, 'smiles_token_mask_enabled', False)
        dropout_enabled = getattr(self.args, 'smiles_token_dropout_enabled', False)
        
        if not mask_enabled and not dropout_enabled:
            return
        
        # Import and create the augmenter
        from swift.llm.dataset.preprocessor.smiles_augmentation import create_smiles_augmenter
        
        mask_ratio = getattr(self.args, 'smiles_token_mask_ratio', 0.15)
        dropout_ratio = getattr(self.args, 'smiles_token_dropout_ratio', 0.1)
        special_tokens = getattr(self.args, 'smiles_special_tokens', None)
        seed = getattr(self.args, 'data_seed', 42)
        
        self.smiles_augmenter = create_smiles_augmenter(
        ...
        self.smiles_augmenter.initialize_with_tokenizer(self.template.tokenizer)
        
        logger.info(f'SMILES token augmenter initialized: mask_enabled={mask_enabled}, '
                   f'mask_ratio={mask_ratio}, dropout_enabled={dropout_enabled}, '
                   f'dropout_ratio={dropout_ratio}')
    ...
    def _compute_acc(self, outputs, labels, cu_seqlens=None, attention_mask=None) -> None:
        ...
        if task_type == 'embedding':
            ...
            if problem_type == 'regression':
                ...
                # Multitask: classification accuracy + regression MSE
                # labels is a dict with 'cls_labels' and 'reg_labels'
                if isinstance(labels, dict):
                    cls_labels = labels.get('cls_labels')
                    reg_labels = labels.get('reg_labels')
                else:
                    # Fallback: assume labels is a tuple/list [cls_labels, reg_labels]
                    try:
                        cls_labels, reg_labels = labels[0], labels[1]
                    except (TypeError, IndexError):
                        # Cannot parse labels, skip accuracy computation
                        return
                
                # Skip if labels are None
                if cls_labels is None and reg_labels is None:
                    return
                
                num_cls_labels = getattr(self.model.config, 'num_cls_labels', None)
                if num_cls_labels is None:
                    num_cls_labels = getattr(args, 'num_cls_labels', logits.shape[-1] // 2)
                num_reg_labels = getattr(self.model.config, 'num_reg_labels', None)
                if num_reg_labels is None:
                    num_reg_labels = getattr(args, 'num_reg_labels', 1)
                
                cls_logits = logits[:, :num_cls_labels]
                reg_logits = logits[:, num_cls_labels:num_cls_labels + num_reg_labels]
                ...
                if cls_labels is not None:
                    ...
                    # Ensure cls_labels is a tensor and on the same device
                    if not isinstance(cls_labels, torch.Tensor):
                        cls_labels = torch.tensor(cls_labels, device=cls_preds.device)
                    else:
                        cls_labels = cls_labels.to(cls_preds.device)
                    cls_acc = (cls_preds == cls_labels).float()
                    metrics['cls_acc'] = cls_acc
                
                # Regression MSE - ensure reg_labels has correct shape and is a tensor
                if reg_labels is not None:
                    if not isinstance(reg_labels, torch.Tensor):
                        reg_labels = torch.tensor(reg_labels, device=reg_logits.device)
                    else:
                        reg_labels = reg_labels.to(reg_logits.device)
                    if reg_labels.dim() == 1:
                        reg_labels = reg_labels.unsqueeze(-1)
                    reg_mse = F.mse_loss(reg_logits, reg_labels, reduction='none').mean(dim=-1)
        ...
        if metrics:
            ...
            # Log dynamic loss weights for multitask training
            if mode == 'train' and task_type == 'seq_cls' and problem_type == 'multitask':
                from .multitask_loss import get_multitask_loss_weighter
                weighter = get_multitask_loss_weighter()
                if weighter is not None:
                    weights = weighter.get_weights()
                    for name, weight in weights.items():
                        self.custom_metrics[mode][f'{name}_weight'].update(torch.tensor(weight))
                    if hasattr(weighter, 'get_uncertainties'):
                        uncertainties = weighter.get_uncertainties()
                        for name, unc in uncertainties.items():
                            self.custom_metrics[mode][f'{name}_uncertainty'].update(torch.tensor(unc))
