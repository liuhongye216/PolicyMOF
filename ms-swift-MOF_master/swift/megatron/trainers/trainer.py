...
class MegatronTrainer(BaseMegatronTrainer):
    ...
    def seq_cls_loss_func(self, output_tensor, *, labels: torch.Tensor, packed_seq_params=None, attention_mask=None):
        ...
        if args.problem_type == 'regression':
            ...
            loss = loss_fct(logits, labels)
        elif args.problem_type == 'multitask':
            # Multitask: classification + regression joint training
            num_cls_labels = getattr(args, 'num_cls_labels', num_labels // 2)
            num_reg_labels = getattr(args, 'num_reg_labels', 1)
            multitask_loss_weight = getattr(args, 'multitask_loss_weight', 1.0)
            ...
            if isinstance(labels, dict):
                ...
                reg_labels = labels['reg_labels']
            else:
                cls_labels = labels[:, 0].long()
                reg_labels = labels[:, 1:].float()
            
            # Classification loss
            cls_loss_fct = CrossEntropyLoss()
            cls_loss = cls_loss_fct(cls_logits, cls_labels.view(-1))
            acc = (cls_logits.detach().argmax(dim=-1) == cls_labels.view(-1)).float().mean()
            
            # Regression loss
            reg_loss_fct = MSELoss()
            reg_loss = reg_loss_fct(reg_logits, reg_labels)
            reg_mse = reg_loss.detach().clone()
