...
class Template(ProcessorMixin):
    ...
    def encode(self,
        ...
        elif self.task_type == 'multi_task_gen':
            task = chosen.extra_kwargs.get('task', 'gen')
            encoded = self._encode_truncated(chosen)
            if task in ('cls', 'reg'):
                encoded.pop('labels', None)
                if chosen.label is not None:
                    encoded['_task_label'] = (
                        int(chosen.label) if task == 'cls'
                        else float(chosen.label))
                encoded['labels'] = [-100] * len(encoded['input_ids'])
            # Always carry per-sample task info so collator can route correctly,
            # even when remove_unused_columns=True (e.g. during inference).
            encoded['_task'] = task
        else:
    ...
    def decode_seq_cls(self, logits: torch.Tensor, top_logprobs: int):
        ...
        if problem_type == 'regression':
            ...
            # For multitask, logits are concatenated: [cls_logits, reg_logits]
            num_cls_labels = getattr(self.config, 'num_cls_labels', None)
            num_reg_labels = getattr(self.config, 'num_reg_labels', 1)
            ...
            cls_logprobs = torch.log_softmax(cls_logits, -1)
            
            # Regression predictions
            reg_preds = reg_logits.squeeze(dim=-1).tolist()
            if not isinstance(reg_preds, list):
                reg_preds = [reg_preds]
            
            # Combine predictions
            preds = [{'cls': cls_pred, 'reg': reg_pred} for cls_pred, reg_pred in zip(cls_preds, reg_preds)]
            logprobs = [self._get_seq_cls_logprobs(cls_pred, cls_logprobs[i], top_logprobs) 
    ...
    def data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        if self.task_type == 'causal_lm':
            ...
            res = self._multi_task_gen_data_collator(batch, padding_to=padding_to)
        else:
            raise ValueError(f'task_type: {self.task_type} is not supported.')
        if not self.remove_unused_columns:
    ...
    def _seq_cls_data_collator(self,
        ...
        if labels:
            ...
            if problem_type == 'regression':
                ...
                # For multitask, labels should be a dict with 'cls' and 'reg' keys
                # e.g., {'cls': 2, 'reg': 0.75} or [cls_label, reg_label]
                cls_labels = []
                reg_labels = []
                for label in labels:
                    if isinstance(label, dict):
                        cls_labels.append(label.get('cls'))
                        reg_labels.append(label.get('reg'))
                    elif isinstance(label, (list, tuple)) and len(label) >= 2:
                        cls_labels.append(label[0])
                        reg_labels.append(label[1])
                    else:
                        raise ValueError(f'Invalid multitask label format: {label}. '
                                         'Expected dict with "cls" and "reg" keys or [cls, reg] list.')
                
                # Build the labels dict for multitask forward
                # Ensure reg_labels is 2D for proper broadcasting with regression head
                reg_labels_tensor = None
                if reg_labels[0] is not None:
                    # Handle both scalar and list regression labels
                    if isinstance(reg_labels[0], (list, tuple)):
                        reg_labels_tensor = torch.tensor(reg_labels, dtype=torch.float32)
                    else:
                        reg_labels_tensor = torch.tensor(reg_labels, dtype=torch.float32).unsqueeze(-1)
                
                labels = {
                    'cls_labels': torch.tensor(cls_labels, dtype=torch.long) if cls_labels[0] is not None else None,
                    'reg_labels': reg_labels_tensor,
                }
            else:
    ...
    def _multi_task_gen_data_collator(self,
                                     batch: List[Dict[str, Any]],
                                     *,
                                     padding_to: Optional[int] = None) -> Dict[str, Any]:
        """Collator for multi_task_gen: mixed gen / cls / reg samples."""
        tasks = []
        for b in batch:
            task = b.pop('_task', None)
            if task is None:
                task = b.get('_extra_kwargs', {}).get('task', 'gen')
            tasks.append(task)
        task_labels = [b.pop('_task_label', None) for b in batch]
        ...
        for task, label in zip(tasks, task_labels):
            cls_labels.append(
                int(label) if task == 'cls' and label is not None else -1)
            reg_labels.append(
                float(label) if task == 'reg' and label is not None else 0.0)

        res['_cls_labels'] = torch.tensor(cls_labels, dtype=torch.long)
        res['_reg_labels'] = torch.tensor(reg_labels, dtype=torch.float32)
        # Always expose per-sample task list so trainer / infer engine can route.
        res['task'] = tasks
        return res
