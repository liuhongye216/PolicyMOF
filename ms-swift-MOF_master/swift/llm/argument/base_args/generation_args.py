...
class GenerationArguments:
    ...
    def get_request_config(self):
        ...
        if task_type not in {'causal_lm', 'multi_task_gen'}:
            return
        from swift.llm import RequestConfig

        max_tokens = self.max_new_tokens
        if max_tokens is None and task_type == 'multi_task_gen':
            # Keep multi-task infer responsive when users forget to set max_new_tokens.
            max_tokens = 256
            logger.info('Setting max_new_tokens to 256 for multi_task_gen infer by default.')
