...
class DataArguments:
    ...
    # SMILES augmentation switch for molecular data
    use_isomeric_smiles: bool = field(
        default=False,
        metadata={
            'help': 'Whether to use isomeric (randomized) SMILES instead of canonical SMILES during training. '
                    'When True, the model will use the "isomeric_smiles" field from the dataset if available. '
                    'This is useful for SMILES augmentation to improve model robustness.'
        }
    )
    
    # SMILES token-level augmentation settings
    smiles_token_mask_enabled: bool = field(
        default=False,
        metadata={
            'help': 'Whether to enable random masking of SMILES tokens during training. '
                    'When enabled, a portion of SMILES tokens will be replaced with [MASK] token.'
        }
    )
    smiles_token_mask_ratio: float = field(
        default=0.15,
        metadata={
            'help': 'The ratio of SMILES tokens to mask when smiles_token_mask_enabled is True. '
                    'Default is 0.15 (15%% of tokens will be masked).'
        }
    )
    smiles_token_dropout_enabled: bool = field(
        ...
        metadata={
            'help': 'Whether to enable random dropout of SMILES token embeddings during training. '
                    'When enabled, embeddings of some SMILES tokens will be zeroed out.'
        }
    )
    smiles_token_dropout_ratio: float = field(
        default=0.1,
        metadata={
            'help': 'The ratio of SMILES token embeddings to dropout when smiles_token_dropout_enabled is True. '
                    'Default is 0.1 (10%% of token embeddings will be zeroed).'
        }
    )
    smiles_special_tokens: Optional[List[str]] = field(
        default=None,
        metadata={
            'help': 'List of special tokens that should NOT be masked or dropped. '
                    'By default, common special tokens like [CLS], [SEP], [PAD], <s>, </s>, etc. are excluded.'
        }
    )
    ...
    def get_dataset_kwargs(self):
        return {
            ...
            'use_isomeric_smiles': self.use_isomeric_smiles,
            # Note: smiles_token_mask_* and smiles_token_dropout_* are NOT passed here
            # They are used by the trainer, not the dataset loader
        }
