...
class ModelArguments:
    ...
    Args:
        ...
        problem_type (Optional[str]): The problem type for classification tasks (`--task_type` 'seq_cls'). Options are
            'regression', 'single_label_classification', 'multi_label_classification', 'multitask'. Defaults to None,
            but is automatically set to 'regression' if the model is a reward_model or `num_labels` is 1, and
            'single_label_classification' otherwise. If set to 'multitask', both classification and regression tasks
            will be jointly trained.
        num_cls_labels (Optional[int]): The number of classification labels for multitask training. Required when
            `problem_type` is 'multitask'. Defaults to None.
        num_reg_labels (Optional[int]): The number of regression outputs for multitask training. Required when
            `problem_type` is 'multitask'. Defaults to 1.
        multitask_loss_weight (float): The weight for regression loss in multitask training (only used when 
            `multitask_loss_strategy` is 'fixed'). The total loss is computed as: 
            cls_loss + multitask_loss_weight * reg_loss. Defaults to 1.0.
        multitask_loss_strategy (str): The strategy for weighting multitask losses. Options:
            - 'fixed': Use `multitask_loss_weight` as a fixed weight (default).
            - 'uncertainty': Kendall et al. 2018 - learns task uncertainty to automatically balance losses.
              The model learns σ (uncertainty) for each task, where higher uncertainty means lower weight.
            - 'dwa': Liu et al. 2019 - Dynamic Weight Average based on loss change rate.
              Weights are computed based on the relative decrease of each task's loss.
        dwa_temperature (float): Temperature for DWA strategy. Higher values lead to more uniform weights.
            Only used when `multitask_loss_strategy` is 'dwa'. Defaults to 2.0.
    ...
    task_type: Literal['causal_lm', 'seq_cls', 'embedding', 'reranker', 'generative_reranker', 'multi_task_gen'] = None
    ...
    problem_type: Literal['regression', 'single_label_classification', 'multi_label_classification', 'multitask'] = None
    # Multitask training parameters
    num_cls_labels: Optional[int] = None  # Number of classification labels for multitask
    num_reg_labels: Optional[int] = 1  # Number of regression outputs for multitask
    multitask_loss_weight: float = 1.0  # Weight for regression loss in multitask training (only for 'fixed' strategy)
    # Dynamic loss weighting strategy: 'fixed', 'uncertainty', 'dwa'
    # - 'fixed': Use multitask_loss_weight as fixed weight
    # - 'uncertainty': Kendall et al. 2018 - learns task uncertainty to weight losses
    # - 'dwa': Liu et al. 2019 - Dynamic Weight Average based on loss change rate
    multitask_loss_strategy: Literal['fixed', 'uncertainty', 'dwa'] = 'fixed'
    dwa_temperature: float = 2.0  # Temperature for DWA strategy (higher = more uniform weights)
    ...
    def get_model_kwargs(self):
        return {
            ...
            'num_reg_labels': self.num_reg_labels,
            'multitask_loss_weight': self.multitask_loss_weight,
        }
