...
class PtEngine(InferEngine):
    ...
    @staticmethod
    def _slice_batched_inputs(inputs: Dict[str, Any], indices: List[int], batch_size: int) -> Dict[str, Any]:
        if not indices:
            return {}
        res = {}
        for k, v in inputs.items():
            if isinstance(v, torch.Tensor):
                # Only slice tensors whose first dim matches batch_size; otherwise
                # this is a shared 1D tensor (e.g. position_ids, logits_to_keep)
                # and must be passed through as-is.
                if v.dim() >= 1 and v.size(0) == batch_size:
                    idx = torch.tensor(indices, device=v.device, dtype=torch.long)
                    res[k] = v.index_select(0, idx)
                else:
                    res[k] = v
            elif isinstance(v, list) and len(v) == batch_size:
                res[k] = [v[i] for i in indices]
            else:
                res[k] = v
        return res

    @staticmethod
    def _pool_last_token(hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        seq_len = mask.size(1)
        last_idx = seq_len - 1 - mask.flip(dims=[1]).argmax(dim=1)
        return hidden_states[torch.arange(hidden_states.shape[0], device=hidden_states.device), last_idx]

    def _infer_multi_task_gen(self, template: Template, inputs: Dict[str, Any], *, generation_config: GenerationConfig,
                              adapter_request: Optional[AdapterRequest], request_config: RequestConfig,
                              template_inputs) -> List[ChatCompletionResponse]:
        task_types = inputs.get('task')
        if not isinstance(task_types, list):
            return self._infer_full(
                template=template,
                inputs=inputs,
                generation_config=generation_config,
        ...
        res: List[Optional[ChatCompletionResponse]] = [None] * batch_size
        gen_indices = [i for i, t in enumerate(task_types) if t == 'gen']
        cls_reg_indices = [i for i, t in enumerate(task_types) if t in {'cls', 'reg'}]

        # Generation branch (lm_head)
        if gen_indices:
            gen_inputs = self._slice_batched_inputs(inputs, gen_indices, batch_size)
            gen_template_inputs = [template_inputs[i] for i in gen_indices]
            gen_res = self._infer_full(
                template=template,
                inputs=gen_inputs,
                generation_config=generation_config,
                adapter_request=adapter_request,
                request_config=request_config,
                template_inputs=gen_template_inputs)
            for i, item in zip(gen_indices, gen_res):
                res[i] = item

        # Classification / regression branch (task heads)
        if cls_reg_indices:
            task_inputs = self._slice_batched_inputs(inputs, cls_reg_indices, batch_size)
            for k in ('task', '_cls_labels', '_reg_labels', 'labels', 'loss_scale', 'compute_loss_func', 'channel'):
                task_inputs.pop(k, None)
            task_inputs['output_hidden_states'] = True

            call_kwargs = {}
            adapter_names = self._get_adapter_names(adapter_request)
            if adapter_names is not None:
                call_kwargs['adapter_names'] = adapter_names
            output = self.model(**task_inputs, **call_kwargs)
            hidden_states = output.hidden_states[-1]
            pooled = self._pool_last_token(hidden_states, task_inputs.get('attention_mask'))
            base_model = template.get_base_model(self.model)
            num_prompt_tokens = self._get_num_tokens(task_inputs)

            # PEFT's ModulesToSaveWrapper sometimes leaves the wrapped head on CPU
            # after checkpoint load, even when the base model is on GPU. Move the
            # head to the right device once, but keep its trained dtype intact.
            def _resolve_head_dtype(head):
                try:
            ...
            if hasattr(base_model, 'cls_head'):
                base_model.cls_head.to(device=target_device)
            if hasattr(base_model, 'reg_head'):
                base_model.reg_head.to(device=target_device)

            for local_i, global_i in enumerate(cls_reg_indices):
                task = task_types[global_i]
                vec = pooled[local_i:local_i + 1]
                if task == 'cls':
                    if not hasattr(base_model, 'cls_head'):
                        raise ValueError('multi_task_gen infer requires model.cls_head for cls samples.')
                    head_dtype = _resolve_head_dtype(base_model.cls_head)
                    pred = int(base_model.cls_head(vec.to(head_dtype)).argmax(dim=-1).item())
                    content = str(pred)
                else:
                    if not hasattr(base_model, 'reg_head'):
                        raise ValueError('multi_task_gen infer requires model.reg_head for reg samples.')
                    head_dtype = _resolve_head_dtype(base_model.reg_head)
                    pred = float(base_model.reg_head(vec.to(head_dtype)).squeeze(-1).item())
                    content = str(pred)
                ...
                choices = [
                    ChatCompletionResponseChoice(
                        index=0,
                        message=ChatMessage(role='assistant', content=content, tool_calls=None),
                        finish_reason='stop',
                        logprobs=None)
                ]
                res[global_i] = ChatCompletionResponse(model=self.model_name, choices=choices, usage=usage_info)

        if any(item is None for item in res):
            raise RuntimeError('multi_task_gen infer produced incomplete results.')
        return res
    ...
    def _infer_full(self, template: Template, inputs: Dict[str, Any], *, generation_config: GenerationConfig,
        ...
        # Drop training-only fields that may exist in multitask collator output.
        # These keys are consumed during training compute_loss, but must never be
        # forwarded to model.generate().
        infer_inputs = dict(inputs)
        for k in ('task', '_cls_labels', '_reg_labels', 'labels', 'loss_scale', 'compute_loss_func', 'channel'):
            infer_inputs.pop(k, None)

        generate_kwargs = {'generation_config': generation_config, **infer_inputs}
        adapter_names = self._get_adapter_names(adapter_request)
        if adapter_names is not None:
            generate_kwargs['adapter_names'] = adapter_names
        num_prompt_tokens = self._get_num_tokens(inputs)
    ...
    def _infer(
        ...
        *,
        ...
        if self.model_info.task_type in {'causal_lm', 'multi_task_gen'}:
        ...
        if request_config.stream:
            ...
            if len(kwargs) > 0:
                if template.task_type in {'seq_cls', 'prm', 'embedding', 'reranker', 'generative_reranker'}:
                    infer_func = self._infer_forward
                elif template.task_type == 'multi_task_gen':
                    infer_func = self._infer_multi_task_gen
                else:
                    infer_func = self._infer_full
