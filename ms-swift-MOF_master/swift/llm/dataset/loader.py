...
def load_dataset(
    ...
    *,
    ...
    Args:
        ...
        use_isomeric_smiles: Whether to use isomeric SMILES instead of canonical SMILES
    Returns:
    ...
    # Apply isomeric SMILES preprocessing if enabled
    # Note: remove_unused_columns should be False when using isomeric SMILES
    # to preserve the isomeric_smiles field until preprocessing
    effective_remove_unused_columns = remove_unused_columns
    if use_isomeric_smiles:
        effective_remove_unused_columns = False
        logger.info('use_isomeric_smiles=True: Using isomeric SMILES for training data augmentation')
    ...
    load_kwargs = {
        ...
        'streaming': streaming,
        'hub_token': hub_token,
        'remove_unused_columns': effective_remove_unused_columns,
    }
    ...
    # Apply isomeric SMILES preprocessing if enabled
    if use_isomeric_smiles:
        from .preprocessor import IsomericSmilesPreprocessor
        isomeric_preprocessor = IsomericSmilesPreprocessor(use_isomeric_smiles=True)
        if train_datasets is not None:
            train_datasets = isomeric_preprocessor(
                train_datasets, num_proc=num_proc or 1, load_from_cache_file=load_from_cache_file, strict=strict)
            # Remove unused columns after preprocessing
            if remove_unused_columns:
                train_datasets = RowPreprocessor.remove_useless_columns(train_datasets)
        # Note: val_datasets should use canonical SMILES (not isomeric) for consistent evaluation
        if val_datasets is not None and remove_unused_columns:
            val_datasets = RowPreprocessor.remove_useless_columns(val_datasets)
