...
def get_model_tokenizer_from_local(model_dir: str,
    ...
    if model_info.task_type == 'seq_cls':
        ...
        # Handle multitask parameters
        if problem_type == 'multitask':
            num_cls_labels = kwargs.get('num_cls_labels')
            num_reg_labels = kwargs.get('num_reg_labels', 1)
            multitask_loss_weight = kwargs.get('multitask_loss_weight', 1.0)
            multitask_loss_strategy = kwargs.get('multitask_loss_strategy', 'fixed')
            dwa_temperature = kwargs.get('dwa_temperature', 2.0)
            if num_cls_labels is not None:
                model_config.num_cls_labels = num_cls_labels
            model_config.num_reg_labels = num_reg_labels
            model_config.multitask_loss_weight = multitask_loss_weight
            model_config.multitask_loss_strategy = multitask_loss_strategy
            model_config.dwa_temperature = dwa_temperature
    ...
    if load_model:
        ...
        if model_info.task_type == 'multi_task_gen':
            from swift.llm.model.patcher import patch_multi_task_gen_heads
            patch_multi_task_gen_heads(model, **kwargs)
...
def get_model_tokenizer(
        ...
        *,
        ...
        automodel_class=None,
        task_type: Literal['causal_lm', 'seq_cls', 'reranker', 'generative_reranker', 'multi_task_gen'] = None,
        num_labels: Optional[int] = None,
        # multitask parameters
        problem_type: Optional[str] = None,
        num_cls_labels: Optional[int] = None,
        num_reg_labels: Optional[int] = None,
        multitask_loss_weight: Optional[float] = None,
    ...
    # Pass multitask parameters
    kwargs['problem_type'] = problem_type
    if num_cls_labels is not None:
        kwargs['num_cls_labels'] = num_cls_labels
    if num_reg_labels is not None:
        kwargs['num_reg_labels'] = num_reg_labels
    if multitask_loss_weight is not None:
        kwargs['multitask_loss_weight'] = multitask_loss_weight
