...
def transformers_seq_cls_forward(self, *args, origin_forward, padding_side=None, **kwargs):
    ...
    # Handle multitask with separate classification and regression heads
    if self.config.problem_type == 'multitask':
        cls_logits = self.score_cls(output.logits)
        reg_logits = self.score_reg(output.logits)
    else:
    ...
    if padding_side == 'left':
        if self.config.problem_type == 'multitask':
            pooled_cls_logits = cls_logits[:, -1]
            pooled_reg_logits = reg_logits[:, -1]
        else:
        ...
        if isinstance(sequence_lengths, torch.Tensor):
            if self.config.problem_type == 'multitask':
                sequence_lengths = sequence_lengths.to(cls_logits.device)
            else:
                sequence_lengths = sequence_lengths.to(logits.device)

        if self.config.problem_type == 'multitask':
            pooled_cls_logits = cls_logits[torch.arange(batch_size, device=cls_logits.device), sequence_lengths]
            pooled_reg_logits = reg_logits[torch.arange(batch_size, device=reg_logits.device), sequence_lengths]
        else:
    ...
    if labels is not None:
        if self.config.problem_type == 'multitask':
            # For multitask, labels is a dict with 'cls_labels' and 'reg_labels'
            ...
            if cls_labels is not None:
                cls_labels = cls_labels.to(pooled_cls_logits.device)
                cls_loss_fct = CrossEntropyLoss()
                cls_loss = cls_loss_fct(pooled_cls_logits.view(-1, self.num_cls_labels), cls_labels.view(-1))
            
            if reg_labels is not None:
                reg_labels = reg_labels.to(pooled_reg_logits.device)
                reg_loss_fct = MSELoss()
                if self.num_reg_labels == 1:
                    reg_loss = reg_loss_fct(pooled_reg_logits.squeeze(), reg_labels.squeeze())
                else:
                    reg_loss = reg_loss_fct(pooled_reg_logits, reg_labels)
            
            # Combine losses with weight
            multitask_loss_weight = getattr(self.config, 'multitask_loss_weight', 1.0)
            if cls_loss is not None and reg_loss is not None:
                loss = cls_loss + multitask_loss_weight * reg_loss
            elif cls_loss is not None:
                loss = cls_loss
            elif reg_loss is not None:
                loss = reg_loss
            
            # Concatenate logits for output (cls_logits first, then reg_logits)
            pooled_logits = torch.cat([pooled_cls_logits, pooled_reg_logits], dim=-1)
        else:
        ...
        # No labels provided, still need to set pooled_logits for multitask
        if self.config.problem_type == 'multitask':
            pooled_logits = torch.cat([pooled_cls_logits, pooled_reg_logits], dim=-1)
...
def patch_multi_task_gen_heads(model, **kwargs):
    """Add classification and regression heads to a CausalLM model for multi-task training.

    The base CausalLM model (with lm_head for generation) is kept intact.
    Additional linear heads are attached for classification and/or regression.
    """
    num_cls_labels = kwargs.get('num_cls_labels')
    num_reg_labels = kwargs.get('num_reg_labels')

    if not num_cls_labels and not num_reg_labels:
        logger.warning('multi_task_gen: neither num_cls_labels nor num_reg_labels specified, '
                       'no task heads will be added.')
        return
    ...
    if num_cls_labels and num_cls_labels > 0:
        model.cls_head = nn.Linear(hidden_size, num_cls_labels, bias=False, dtype=dtype)
        if model.cls_head.weight.device == torch.device('meta'):
            model.cls_head.to_empty(device='cpu')
        model.cls_head.weight.data.normal_(mean=0.0, std=initializer_range)
        model.config.num_cls_labels = num_cls_labels
        logger.info(f'multi_task_gen: added cls_head  hidden_size={hidden_size} -> {num_cls_labels}')

    if num_reg_labels and num_reg_labels > 0:
        model.reg_head = nn.Linear(hidden_size, num_reg_labels, bias=False, dtype=dtype)
        if model.reg_head.weight.device == torch.device('meta'):
            model.reg_head.to_empty(device='cpu')
        model.reg_head.weight.data.normal_(mean=0.0, std=initializer_range)
        model.config.num_reg_labels = num_reg_labels
        logger.info(f'multi_task_gen: added reg_head  hidden_size={hidden_size} -> {num_reg_labels}')
...
def _patch_sequence_classification(model, model_meta):
    ...
    # Check if this is a multitask problem
    problem_type = getattr(model.config, 'problem_type', None)
    if problem_type == 'multitask':
        # Multitask: create separate classification and regression heads
        num_cls_labels = getattr(model.config, 'num_cls_labels', None)
        num_reg_labels = getattr(model.config, 'num_reg_labels', 1)
        
        if num_cls_labels is None:
            raise ValueError('num_cls_labels must be specified for multitask problem_type')
        
        llm_model.num_cls_labels = num_cls_labels
        llm_model.num_reg_labels = num_reg_labels
        
        # Classification head
        llm_model.score_cls = nn.Linear(hidden_size, num_cls_labels, bias=False, dtype=llm_model.dtype)
        if llm_model.score_cls.weight.device == torch.device('meta'):
            llm_model.score_cls.to_empty(device='cpu')
        llm_model.score_cls.weight.data.normal_(mean=0.0, std=initializer_range)
        
        # Regression head
        llm_model.score_reg = nn.Linear(hidden_size, num_reg_labels, bias=False, dtype=llm_model.dtype)
        if llm_model.score_reg.weight.device == torch.device('meta'):
            llm_model.score_reg.to_empty(device='cpu')
        llm_model.score_reg.weight.data.normal_(mean=0.0, std=initializer_range)
        
        # Create a dummy score for compatibility (not used in multitask)
        llm_model.score = nn.Linear(hidden_size, num_cls_labels + num_reg_labels, bias=False, dtype=llm_model.dtype)
        ...
        logger.info(f'Multitask heads created: cls_labels={num_cls_labels}, reg_labels={num_reg_labels}')
    else:
        # Standard single-task head
